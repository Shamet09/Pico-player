...
/* PATCH PicoPlayer: aggiunto HELIX_PORTABLE_C.
 * Il ramo "__GNUC__ && ARM" piu' sopra usa smull e operandi con barrel
 * shifter (eor %0,%2,%2,asr #31): istruzioni ARMv5TE/DSP che il Cortex-M0+
 * dell'RP2040 (ARMv6-M) NON ha, quindi definire ARM farebbe fallire la
 * compilazione. Definiamo invece HELIX_PORTABLE_C (vedi CMakeLists.txt) per
 * usare la versione C portabile. */
#elif defined(HELIX_PORTABLE_C) || defined(ARDUINO) || defined(ESP32) || defined(ESP_PLATFORM) || defined(__APPLE__) || defined(__unix__)
